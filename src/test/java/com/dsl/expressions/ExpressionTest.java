...
class ExpressionTest {
    ...
    void pathWithTwoNodesRight() {
        String s = node("s", "Name").rightNode("b").rightNode("c", "Person").asString();
        Assertions.assertEquals("(s:Name)-->(b)-->(c:Person)", s);
    }

    @Test
    void pathWithTwoNodesLeft() {
        String s = node("s", "Name").leftNode("b").leftNode("c", "Person").asString();
        Assertions.assertEquals("(s:Name)<--(b)<--(c:Person)", s);
    }

    @Test
    void pathWithTwoNodes() {
        String s = node("s", "Name").toNode("b").toNode("c", "Person").asString();
        Assertions.assertEquals("(s:Name)--(b)--(c:Person)", s);
    }

    @Test
    void pathWithRelationship() {
        String s = node("s", "Name").toRel().to().asString();
        Assertions.assertEquals("(s:Name)-[]-()", s);
    }

    @Test
    void pathWithAliasRelationship() {
        String s = node("s", "Name").toRel("a").to().asString();
        Assertions.assertEquals("(s:Name)-[a]-()", s);
    }

    @Test
    void pathWithAliasRelationshipRight() {
        String s = node("s", "Name").right("a").to().asString();
        Assertions.assertEquals("(s:Name)-[a]->()", s);
    }

    @Test
    void pathWithAliasRelationshipLeft() {
        String s = node("s", "Name").left("a").to().asString();
        Assertions.assertEquals("(s:Name)<-[a]-()", s);
    }

    @Test
    void pathRelationshipRight() {
        String s = node("s", "Name").right().to().asString();
        Assertions.assertEquals("(s:Name)-[]->()", s);
    }
